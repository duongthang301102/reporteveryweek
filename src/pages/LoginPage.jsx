...
import { getAllUsers } from '../db'; // Import hàm lấy user từ DB
...
const LoginPage = ({ onLogin }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
        // Lấy danh sách user mới nhất từ Firebase
        const users = await getAllUsers();
        
        // [AN TOÀN] Nếu chưa có user nào (lần đầu chạy trên DB mới), tạo admin ảo trong bộ nhớ để đăng nhập lần đầu
    ...
    <div className="flex min-h-screen w-full items-center justify-center bg-[#0F1115] relative overflow-hidden px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-[#1C1E26]/90 backdrop-blur-xl p-6 md:p-10 rounded-2xl shadow-2xl border border-gray-800 w-full max-w-[420px] relative z-10 animate-fade-in-up transition-all duration-300">
        ...
        <div className="text-center mb-8 md:mb-10">
            ...
            <p className="text-gray-500 text-xs md:text-sm font-medium">Đăng nhập để truy cập dữ liệu (Cloud)</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5 md:space-y-6">
            ...
            <button 
                type="submit" 
                disabled={loading}
                className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold py-3.5 md:py-4 rounded-xl shadow-lg shadow-blue-500/25 transition-all transform active:scale-[0.98] mt-2 text-sm md:text-base tracking-wide flex items-center justify-center gap-2"
            >
                {loading ? <><Loader2 className="animate-spin" size={20}/> Đang xác thực...</> : "ĐĂNG NHẬP HỆ THỐNG"}
            </button>
        </form>
