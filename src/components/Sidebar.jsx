...
import { ShieldCheck, Table2, Database, ClipboardList, X, FileSpreadsheet, Mail } from 'lucide-react';
import logoImage from '../assets/logo.png'; 

// Import Modal Mật Khẩu
import PasswordModal from './PasswordModal';

const MenuItem = ({ icon, label, path, active, onClick, className }) => (
  <div onClick={onClick} className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 ${active ? 'bg-[#2D3039] text-blue-400 border-l-4 border-blue-500' : 'text-gray-400 hover:bg-[#1C1E26] hover:text-gray-200'} ${className || ''}`}>
    {icon} <span className={`font-medium text-sm ${active ? 'font-bold' : ''}`}>{label}</span>
  </div>
);
...
const Sidebar = ({ onClearData, onSendEmail, isOpen, onClose }) => {
  ...
  // State quản lý Modal Mật khẩu cho Email
  ...
  const menuItems = [
    ...
    { label: 'Combo', path: '/services', icon: <ClipboardList size={20} /> },
    // Đã ẩn 2 menu Data và Test
  ];

  const handleNavigate = (path) => { navigate(path); if (onClose) onClose(); };

  // 1. KHI BẤM NÚT GỬI EMAIL -> CHỈ MỞ MODAL
  const handleSendClick = () => {
      setShowEmailAuth(true); // Mở bảng nhập pass
  };

  // 2. KHI NHẬP ĐÚNG PASS -> THỰC HIỆN GỬI
  const handleEmailAuthSuccess = async () => {
      setIsSending(true);
      await onSendEmail(); // Gọi hàm gửi thật sự
      setIsSending(false);
      if(onClose) onClose();
  };
    ...
    <>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-[#0F1115] flex flex-col border-r border-gray-800 transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        ...
        <nav className="flex-1 px-3 md:px-4 space-y-1 md:space-y-2 mt-2 overflow-y-auto">
          ...
          {menuItems.map((item) => (<MenuItem key={item.label} icon={item.icon} label={item.label} active={location.pathname === item.path} onClick={() => handleNavigate(item.path)} className={item.className} />))}
          
          {/* NÚT GỬI EMAIL (ĐÃ BẢO MẬT) */}
          <div className="mt-4 pt-4 border-t border-gray-800">
             <p className="text-[10px] md:text-xs text-gray-500 uppercase font-semibold px-4 mb-2 tracking-wider">Tiện ích</p>
             <MenuItem 
                icon={<Mail size={20} className={isSending ? "animate-pulse" : ""} />} 
                label={isSending ? "Đang gửi..." : "Gửi Báo Cáo Email"} 
                onClick={handleSendClick} // Mở bảng nhập pass
                className="text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10" 
             />
          </div>
        </nav>

        {/* NÚT XÓA DỮ LIỆU (Đã được bảo mật ở ConfirmModal cũ) */}
      ...
      {/* --- MODAL BẢO MẬT CHO EMAIL --- */}
      <PasswordModal 
        isOpen={showEmailAuth} 
        onClose={() => setShowEmailAuth(false)}
        onSuccess={handleEmailAuthSuccess} // Chạy hàm gửi khi đúng pass
        title="Xác thực gửi Email"
      />
    </>
  );
};
